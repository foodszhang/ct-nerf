...
class TIGREDataset_MLG(Dataset):
    ...
    def __init__(
        ...
        if type == "train":
            ...
            self.dif_projs = torch.tensor(
                data["val"]["dif_projections"], dtype=torch.float32, device=device
            )
    ...
    def __getitem__(self, index):
        if self.type == "train":
            ...
            out = {"projs": projs, "rays": rays, "projections": self.projs}
            return out
